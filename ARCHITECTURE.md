...
管理控制台位于 `web/`，生产构建由同一个 Go 服务在 `/console/` 提供。控制台包含待处理队列、已处理审核历史、客户端管理、Webhook 投递和运营概览五个视图；顶层工作区只负责视图切换和 JWT 会话边界，各视图通过独立 composable 管理请求状态。运营概览复用 `GET /api/v1/reviews/stats`，展示审核最终决策、人工复核和 Webhook 最新状态的累计快照，不把累计数描述成时间趋势。Webhook 投递视图复用管理员 delivery API，可按状态、客户端和请求 ID 查询最新 delivery 状态，并对失败记录执行二次确认的手动重试；并发冲突会刷新列表。当前后端记录是每个 delivery 的最新状态，不是逐次尝试明细表。投递错误在持久化入口和运营输出边界都会归一化为有限类别，避免暴露 URL 查询参数、底层网络请求文本或数据库错误详情。
...
当前已记录 HTTP 请求、文本审核结果、人工复核完成延迟、Webhook 投递结果与耗时，以及后台重试批次结果。这些 Prometheus 指标只使用固定枚举标签，不包含请求 ID、客户端 ID、URL 或错误文本。控制台运营概览另从持久化记录计算累计业务快照；更完整的时间趋势、失败分类和告警规则仍需后续补齐并验证。
...
- 带时间窗口的趋势报表、更完整的失败分类和告警规则。
- Webhook 逐次尝试明细历史和更完整的投递诊断界面。
