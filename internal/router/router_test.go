...
func TestSetupRequiresAdminForClientRoutes(t *testing.T) {
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("client detail status = %d, want 403", recorder.Code)
	}
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/clients/11/name", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	recorder = httptest.NewRecorder()
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("client name status = %d, want 403", recorder.Code)
	}

	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/clients/11/policy", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	recorder = httptest.NewRecorder()
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("client policy status = %d, want 403", recorder.Code)
	}

	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/clients/11/webhook", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	recorder = httptest.NewRecorder()
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("client webhook status = %d, want 403", recorder.Code)
	}
