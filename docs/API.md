...
- **认证方式**: JWT Bearer Token；`POST /moderation/check` 和 `GET /moderation/results/:request_id` 也支持外部客户端 API Key
...
- `policy_version`: 可选，客户端审核使用的策略版本；必须匹配 `moderation.policy.version` 或 `moderation.policies[].version`，为空则使用默认策略。
...
查询单个外部客户端的当前状态、API Key 前缀、Webhook URL 和策略版本。响应不会返回 API Key 哈希、完整 API Key 或 Webhook secret。

**端点**: `GET /admin/clients/:id`
...
更新客户端展示名称。该接口不会改变客户端状态、API Key、Webhook 或策略版本，也不会返回 API Key 哈希或 Webhook secret。

**端点**: `POST /admin/clients/:id/name`
...
当前接口为同步处理，支持 JWT 认证或外部客户端 API Key 认证。API Key 客户端配置了 `webhook_url` 时，`allow` / `block` 会同步尝试发送最终决策回调；`review` 会先进入人工复核队列，待管理员复核后再发送最终决策回调。回调投递结果会记录为审计状态，失败投递支持后台自动重试和管理员手动重试。
...
- 外部客户端通过 API Key 调用 `POST /moderation/check` 或 `GET /moderation/results/:request_id` 时，按客户端 ID 限流。
