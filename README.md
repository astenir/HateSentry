...
版本变化见 [`CHANGELOG.md`](CHANGELOG.md)。正式版本镜像发布到 `ghcr.io/astenir/hatesentry`；部署时应固定工作流输出的 `sha256` digest，不要只依赖可变标签。正式部署前请执行 [`MVP 发布验收清单`](docs/release-checklist.zh-CN.md)，不要直接使用本地开发默认密码或示例 provider key。
...
以下值是本地开发示例，不是生产配置。正式发布必须使用 `config/release.env.example` 和发布预检。
...
DB_HOST=localhost
DB_PORT=3306
DB_USERNAME=hatesentry
DB_PASSWORD=hatesentry
...
业务流程完成后，烟测还会使用一致性 MySQL dump 将临时数据库恢复到第二个临时数据库，并核对用户、客户端、审核请求、审核结果、复核案件和 Webhook delivery 表的记录数。两个临时数据库和 dump 会自动清理。

发布环境配置可从 `config/release.env.example` 开始准备，并在启动前执行：

```bash
RELEASE_ENV_FILE=/secure/path/hatesentry-release.env make release-preflight
```

空数据库首次初始化时额外设置 `RELEASE_BOOTSTRAP=1`；初始化管理员后必须清空 `ADMIN_BOOTSTRAP_TOKEN` 并重新执行预检。
...
- [docs/release-checklist.zh-CN.md](docs/release-checklist.zh-CN.md) - MVP 发布验收与回滚清单
